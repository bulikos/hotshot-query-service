...
use crate::{
    ...
    Block, Deltas, NotFoundSnafu, QueryError, QueryResult, QueryableBlock, Resolvable,
};
...
use fetching::{BlockFetcher, Resource};
use futures::{
    channel::oneshot,
...
use parse::{parse_block, parse_leaf, BLOCK_COLUMNS, LEAF_COLUMNS};
use snafu::OptionExt;
use std::{ops::RangeBounds, pin::Pin};
use tokio_postgres::{
    types::{BorrowToSql, ToSql},
    Client, NoTls, RowStream, ToStatement,
};
use versioned_channel::VersionedChannel;
    ...
    Deltas<Types, I>: Resolvable<Block<Types>>,
{
    ...
    async fn get_proposals(
        ...
        let mut query = format!("SELECT {LEAF_COLUMNS} FROM leaf WHERE proposer = $1");
