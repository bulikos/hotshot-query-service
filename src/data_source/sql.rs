...
use super::{
    versioned_channel::{VersionedChannel, VersionedReceiver},
    VersionedDataSource,
};
use crate::{
    availability::{
        AvailabilityDataSource, BlockId, BlockQueryData, ErrorContext, ErrorEvent, LeafId,
        LeafQueryData, ResourceId, TransactionHash, TransactionIndex, UpdateAvailabilityData,
    },
    ...
    Deltas<Types, I>: Resolvable<Block<Types>>,
{
    ...
    {
        Resource::<Types, I>::get(&self.client, id).await
    }
    ...
    where
        R: RangeBounds<usize> + Send,
    {
        Resource::<Types, I>::get_range(&self.client, range).await
    }
    ...
    async fn subscribe_leaves(&self, height: usize) -> QueryResult<Self::LeafStream> {
        Ok(Resource::<Types, I>::subscribe(
            self.client.clone(),
            self.error_stream.clone(),
            self.leaf_stream.subscribe().await,
            height,
        )
        .await)
    }
    ...
    async fn subscribe_blocks(&self, height: usize) -> QueryResult<Self::BlockStream> {
        let fetcher = BlockFetcher {
            blocks: self.block_stream.subscribe().await,
            leaves: self.leaf_stream.subscribe().await,
        };
        Ok(Resource::<Types, I>::subscribe(
            self.client.clone(),
            self.error_stream.clone(),
...
#[async_trait]
trait Resource<Types, I>: Clone + Sized + 'static
where
    Types: NodeType,
    I: NodeImplementation<Types>,
{
...
struct BlockFetcher<Types, I>
where
    Types: NodeType,
    I: NodeImplementation<Types>,
    Block<Types>: QueryableBlock,
{
    blocks: VersionedReceiver<BlockQueryData<Types>>,
    leaves: VersionedReceiver<LeafQueryData<Types, I>>,
}
